# Fabricación de la urna

## Fabricantes y configuración

- Fabricante o responsable comercial de la urna: **Elite Reef Kanarias S.L.**
- Cristalería o proveedor del vidrio: **Cristalería Drago**, según el logotipo visible en las etiquetas de la fotografía
- Tipo: urna marina AIO artesanal con compartimento técnico lateral derecho
- Disposición: tipo península adaptada a una instalación junto a pared
- Medida exterior nominal: **750 × 320 × 400 mm**
- Silicona: **negra**


## Piezas documentadas

| Pieza | Medida / material | Seguridad |
| --- | --- | --- |
| Base | **736 × 306 mm**, vidrio transparente de **10 mm** | Alta |
| Frontal | **750 × 400 mm**, vidrio transparente de **6 mm** | Alta |
| Trasera | **750 × 400 mm**, vidrio transparente de **6 mm** | Alta |
| Laterales cortos | Calculados como **306 × 400 mm**, vidrio de **6 mm** | Alta |
| Separador técnico | Lacobel negro con dos perforaciones circulares | Alta para la identificación; función pendiente |

Con frontal y trasera exteriores, laterales cortos entre ambas piezas y base encajada dentro del perímetro vertical, las medidas observadas son coherentes con:

```text
Largo interior de base: 750 - 6 - 6 - 1 - 1 = 736 mm
Fondo interior de base: 320 - 6 - 6 - 1 - 1 = 306 mm
Laterales cortos: 306 × 400 mm en 6 mm
Base: 736 × 306 mm en 10 mm
Frontal y trasera: 750 × 400 mm en 6 mm
```

La junta de **1 mm** por unión es una consideración de corte y montaje, no una medida verificada de una junta ya ejecutada.

## Conteo visual de piezas en la fotografía

La fotografía [Piezas de vidrio y panel negro](fotos-fabricacion/e610a0c9-608c-44b8-8823-3d3f330df4f4.jpeg) permite contar **9 piezas aparentes**, incluido el panel Lacobel negro. Varias piezas están apiladas o parcialmente superpuestas, por lo que el conteo y la asignación siguiente son visuales:

| Conteo | Identificación posible | Estado |
| ---: | --- | --- |
| 1–2 | Frontal y trasera transparentes de 750 × 400 mm | Coherente con la documentación de corte |
| 3 | Base transparente | Coherente con la documentación de corte |
| 4–5 | Laterales cortos transparentes | Coherente con la geometría exterior |
| 6 | Panel Lacobel negro perforado | Identificación visual confirmada |
| 7–8 | Dos separadores internos transparentes | Compatible con una zona técnica de tres cámaras |
| 9 | Pieza transparente grande adicional | Función sin confirmar; podría ser una tapa |

La pieza adicional no debe asignarse definitivamente como tapa sin comprobar sus medidas, cantos, taladros, apoyos y encaje sobre la urna. Otras posibilidades son una pieza de refuerzo, una división interna adicional o un panel fabricado para otra posición. El plano de corte o la confirmación de Elite Reef Kanarias debe resolverlo.

## Lectura ampliada de las etiquetas

La ampliación de la misma fotografía permite recuperar información adicional de las etiquetas. La perspectiva, el desenfoque y la superposición de las piezas impiden leer todos los códigos completos, pero sí permiten distinguir varios campos:

| Lectura visible | Interpretación | Confianza |
| --- | --- | --- |
| **CRISTALERÍA DRAGO** | Logotipo o referencia visible de la cristalería en las etiquetas | Media-alta; la lectura procede del logotipo de la fotografía |
| **ELITE REEF KANARIAS S.L.** | Nombre que también aparece en las etiquetas, probablemente como cliente, pedido o responsable comercial | Alta |
| **TRANSPARENTE 6 mm** | Vidrio transparente de 6 mm | Alta en la etiqueta superior derecha |
| **CANTO PULIDO BRILLO** | Acabado de canto pulido brillante | Alta en la etiqueta superior derecha |
| **1 de 1 - 750 × 400** | Una pieza de 750 × 400 mm | Alta en la etiqueta superior derecha |
| **1 de 1 - 736 × 306** | Una pieza de 736 × 306 mm, coherente con la base | Media-alta; etiqueta invertida y parcialmente borrosa |
| **306 × 400** | Medida compatible con un lateral corto | Media; parte del texto queda tapada |
| Códigos numéricos y referencias de pedido | Identificadores internos de cristalería | Baja; no se transcriben como dato técnico |

La etiqueta de **750 × 400 mm** refuerza la identificación de una pieza de frontal o trasera en vidrio transparente de 6 mm. La etiqueta de **736 × 306 mm** refuerza la identificación de la base en 10 mm por su correspondencia con la geometría interior y con la pieza observada, aunque el espesor no se lee con la misma claridad en esa etiqueta. La pieza de **306 × 400 mm** es compatible con un lateral corto, pero su función concreta debe confirmarse por posición y medidas físicas.

La fotografía permite asociar el vidrio etiquetado con **Cristalería Drago**, pero no muestra por sí sola el alcance exacto de su trabajo ni permite determinar si fabricó, cortó, pulió o suministró todas las piezas. **Elite Reef Kanarias S.L.** aparece también en las etiquetas y queda identificada como responsable comercial o de la urna. Estas lecturas proceden de una fotografía, no de un plano de fabricación. La medida, el espesor y la función de cada pieza deben prevalecer cuando exista documentación original.

## Separador Lacobel

El panel negro identificado es **Lacobel**. Su uso aporta ocultación visual de la zona técnica, pero la cara lacada y los cantos deben ser compatibles con inmersión permanente en agua salada. Elite Reef Kanarias debe confirmar:

- Orientación de la cara lacada
- Protección de los cantos
- Compatibilidad de Lacobel y silicona con agua salada
- Seguridad para la fauna
- Si el panel es estructural o añadido

## Perforaciones y entrada de agua

El panel tiene dos perforaciones circulares. Sus medidas, cotas y funciones no están confirmadas.

| Perforación | Lectura visual | Función |
| --- | --- | --- |
| Grande | Zona superior o media-alta del panel | Sin confirmar |
| Pequeña | Zona aparentemente más baja | Sin confirmar |

Una imagen muestra una rejilla o peine circular que parece rondar los **65 mm** de diámetro exterior. Esa lectura no permite distinguir entre diámetro de rejilla, accesorio y perforación del vidrio. No se deben comprar pasamuros ni tuberías a partir de esta estimación.

## Apoyo

La base se apoyará sobre una pieza continua de EVA BIBODU de **75 × 32 cm** y **5 mm**, recortada de un rollo de **150 × 32 cm** y densidad declarada de **80 kg/m³**. La ficha completa está en [Espuma de apoyo](espuma.md).

La EVA debe colocarse sin pliegues, cortes, solapes ni bordes levantados, sobre un tablero limpio, rígido, plano y nivelado. No corrige un tablero combado ni una estructura insuficiente. El fabricante de la urna debe aceptar expresamente el material y el espesor.

## Fotos del proceso

Las fotos de fabricación se guardan en [fotos-fabricacion](fotos-fabricacion/). Las dos imágenes disponibles muestran:

- [Piezas de vidrio y panel negro](fotos-fabricacion/e610a0c9-608c-44b8-8823-3d3f330df4f4.jpeg): conjunto de piezas cortadas, etiquetas de cristalería y panel negro perforado separado
- [Urna ensamblada](fotos-fabricacion/e1b59261-bc84-45f3-a7ae-64dff59cd621.jpeg): urna montada sobre una mesa de trabajo, con silicona negra visible y el compartimento técnico lateral integrado

Las fotografías permiten documentar el estado visual de las piezas y del montaje, pero no sustituyen las cotas del fabricante. Cada imagen deberá conservar su fecha, origen y alcance probatorio.


## Acabado pintado mencionado en audio

El último audio de fabricación indica que se pintará la urna. El audio no permite confirmar por sí solo:

- Si se pintará únicamente el compartimento técnico o alguna otra superficie
- Si el acabado será negro
- Qué producto se utilizará
- Qué caras quedarán en contacto permanente con agua salada

La interpretación de que se trata del compartimento técnico y de que el acabado será negro se mantiene como hipótesis de trabajo, no como especificación confirmada. Elite Reef Kanarias debe confirmar la superficie, el color, el producto, el método de aplicación, el sellado y la compatibilidad con agua salada y fauna.

## Pendiente de verificación

- Plano final de corte y montaje
- Medidas internas reales
- Identificación y función de la novena pieza transparente grande
- Confirmar si la pieza adicional es una tapa y si la urna se instalará tapada
- Diámetro de la rejilla y de cada perforación del vidrio
- Función de cada perforación
- Altura de entrada y retorno
- Pulido de los cantos de los agujeros
- Orientación y protección del Lacobel
- Superficie, color y producto del acabado pintado mencionado en el audio
- Compatibilidad del acabado pintado con agua salada y fauna
- Aceptación de la EVA de 5 mm como apoyo
- Nivelación final del tablero antes del llenado
